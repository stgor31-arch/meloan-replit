...
### Для кредитора
- Регистрация кредитора через Яндекс ID
...
- Отправка приглашения заёмщику через Telegram или копирование ссылки
...
**Роли пользователей:**
- **Кредитор (Lender)** — требует аутентификации через Яндекс ID
...
│   └── replit_integrations/
│       └── auth/                 # Express sessions + legacy auth helpers
├── shared/
...
### Яндекс ID (основной способ)
- Кнопка входа размещена на странице `/login`
- Флоу: `GET /api/auth/yandex` → Яндекс ID → `GET /api/auth/yandex/callback`
- Сервер получает профиль пользователя через `login.yandex.ru/info`
...
| GET | `/api/auth/yandex/callback` | Callback Яндекс ID |
| GET | `/api/auth/providers` | Доступные российские OAuth-провайдеры |
...
| `YANDEX_CLIENT_ID` | Client ID приложения Яндекс ID |
| `YANDEX_CLIENT_SECRET` | Client secret приложения Яндекс ID |
| `TELEGRAM_BOT_TOKEN` | Токен Telegram-бота для служебных сценариев |
| `TELEGRAM_BOT_USERNAME` | Имя Telegram-бота без @ |
...
Приложение рассчитано на деплой на российский VPS и доступно по адресу [meloan.ru](https://meloan.ru).
